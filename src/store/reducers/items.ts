...
const itemsSlice = createSlice({
  name: 'items',
  initialState,
  // We reduce anything when we have something to change in this specific state
  // If we aren't changing anything, the reducers must be EMPTY
  reducers: {
    // This is an ACTION:
    isFavorite: (state, { payload }: PayloadAction<string>) => {
      state.map((item) => {
...
// ATIVIDADE: Criar uma action chamada “incrementar” dentro de um slice chamado “contador”.
...
// TYPE: ReducerName/ActionCreated

export const {isFavorite} = itemsSlice.actions;
